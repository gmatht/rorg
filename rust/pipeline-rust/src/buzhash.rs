//! Rolling buzhash primitives matching [src/borg/chunkers/buzhash.pyx](D:/GitHub/borg/src/borg/chunkers/buzhash.pyx).
//! Full content-defined chunk cutting stays in Cython until the buffered reader logic is ported.

use pyo3::prelude::*;
use pyo3::types::PyBytes;

/// Same table as `table_base` in `buzhash.pyx`.
const TABLE_BASE: [u32; 256] = [
...
#[inline]
fn barrel_shift(v: u32, shift: u32) -> u32 {
    let shift = shift & 0x1f;
    let r = (32u32 - shift) & 0x1f;
    (v << shift) | (v >> r)
}

pub fn init_table(seed: u32) -> [u32; 256] {
    let mut t = [0u32; 256];
    for i in 0..256 {
        t[i] = TABLE_BASE[i] ^ seed;
    }
    t
}

/// `_buzhash` from `buzhash.pyx`.
pub fn buzhash_raw(data: &[u8], table: &[u32; 256]) -> u32 {
    let len = data.len();
    if len == 0 {
        return 0;
    }
    ...
    let mut off = 0usize;
    while i > 0 {
        let imod = (i as u32) & 0x1f;
        sum ^= barrel_shift(table[data[off] as usize], imod);
        off += 1;
        i -= 1;
    }
    sum ^ table[data[off] as usize]
}

/// `_buzhash_update` from `buzhash.pyx`.
pub fn buzhash_update_raw(sum: u32, remove: u8, add: u8, window_len: usize, table: &[u32; 256]) -> u32 {
    let lenmod = (window_len & 0x1f) as u32;
    barrel_shift(sum, 1) ^ barrel_shift(table[remove as usize], lenmod) ^ table[add as usize]
}

#[pyfunction(name = "buzhash", signature = (data, seed))]
fn buzhash_py(data: &Bound<'_, PyBytes>, seed: u32) -> u32 {
    let table = init_table(seed);
    buzhash_raw(data.as_bytes(), &table)
}

#[pyfunction(name = "buzhash_update", signature = (sum, remove, add, len, seed))]
fn buzhash_update_py(sum: u32, remove: u8, add: u8, len: usize, seed: u32) -> u32 {
    let table = init_table(seed);
    buzhash_update_raw(sum, remove, add, len, &table)
}

pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(buzhash_py, m)?)?;
    m.add_function(wrap_pyfunction!(buzhash_update_py, m)?)?;
    Ok(())
}
